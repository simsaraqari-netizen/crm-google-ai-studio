      ...
      try {
        ...
        if (initialSession?.user) {
          ...
          if (userData) {
            ...
            setUser(mappedUser);
            // Auto-show properties list for admin/super_admin/employee without requiring manual search
            if (mappedUser.role === 'super_admin' || mappedUser.role === 'admin' || mappedUser.role === 'employee') {
              setHasSearched(true);
            }
      ...
      try {
        if (session?.user) {
          ...
          if (userData) {
            ...
            setUser(mappedUser);
            // Auto-show properties for admin/super_admin/employee after login
            if (mappedUser.role === 'super_admin' || mappedUser.role === 'admin' || mappedUser.role === 'employee') {
              setHasSearched(true);
            }
            if (mappedUser.companyId) {
    ...
    return sourceProperties.filter(p => {
      ...
      const matchesSingle = (q: string) => {
        const nameCodeMatches = 
          searchMatch(p.name, q, p) ||
          searchMatch(p.area, q, p) ||
          searchMatch(p.sector || '', q, p) ||
          searchMatch(p.block || '', q, p) ||
          searchMatch(p.plot_number || '', q, p) ||
          searchMatch(p.house_number || '', q, p) ||
          searchMatch(getPropertyCode(p), q, p) ||
          searchMatch(p.id, q, p) ||
          searchMatch(p.assigned_employee_name || '', q, p);
        
        const phoneMatch = 
          searchMatch(p.phone, q, p) || 
          searchMatch(p.phone_2 || '', q, p);
